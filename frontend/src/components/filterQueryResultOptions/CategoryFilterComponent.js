...
const CategoryFilterComponent = ({ setCategoriesFromFilter }) => {
  ...
  const myRefs = useRef([]); //want to persist values between renders so use useRef
  ...
  const selectCategory = (e, category, idx) => {
    ...
    var selectedMainCategory = category.name.split("/")[0]; //the category is first part in the menu
    var allCategories = myRefs.current.map((_, id) => {  //map through all categories, get name and index
